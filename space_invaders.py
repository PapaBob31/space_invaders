...
class Invaders:
    ...
    def check_position(self):
        for no in self.dead_invaders:
            for coordinate in reversed(self.coordinates):
                for pos in coordinate:
                    if pos[2] == no:
                        self.bullet_pos.append(pos)
                        no = 13
         
    def create_bullets(self):
        if self.coordinates:
            self.bullet_pos = self.coordinates[-1][:]
            if len(self.coordinates) > 1:
                if len(self.coordinates[-1]) < 11:
                    if self.dead_invaders:
                        self.check_position()
        ...
        for invader in pos_lists:
            for bullet in defender.bullets:
                if bullet[0] in range(invader[0], invader[0]+40):
                    if bullet[1] <= invader[1]:
                        if invader in enemy.coordinates[-1]:
                            enemy.dead_invaders.append(invader[2])
